...
# Read the dataset
data <- read.csv("D:/16GBBACKUPUSB/BACKUP_USB_SEPTEMBER2014/SUMMER_STUDENT_2025/HRS_NHANES_HEI_EPICLOCKS_MORT/FINAL_DATA/NHANES/cleaned_results_TABLE5_MODEL1.csv")

# Rename columns for clarity
colnames(data) <- c("Variable", "LnHR", "SE")

# Convert to HR scale and compute 95% CI
data$HR    <- exp(data$LnHR)
data$Lower <- exp(data$LnHR - 1.96 * data$SE)
data$Upper <- exp(data$LnHR + 1.96 * data$SE)

# Optional: order variables (useful for forest plot style)
data$Variable <- factor(data$Variable, levels = rev(data$Variable))

# Plot
ggplot(data, aes(x = Variable, y = HR)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = Lower, ymax = Upper), width = 0.2) +
  geom_hline(yintercept = 1, linetype = "dashed") +  # Null line at HR=1
  coord_flip() +  # Better for readability (forest plot style)
  theme_minimal() +
  labs(
...
***********************SENSITIVITY ANALYSIS 1: COX MODELS*************************
capture log close

capture log using "E:\16GBBACKUPUSB\BACKUP_USB_SEPTEMBER2014\SUMMER_STUDENT_2025\HRS_NHANES_HEI_EPICLOCKS_MORT\OUTPUT\NHANES_HEATMAPCOX_SENS.smcl",replace
...
foreach x of varlist zHorvathAgeEAA zHannumAgeEAA zPhenoAgeEAA zGrimAgeMortEAA zDunedinPoAm  zhei2015_total_score  zses { 
        ...
        * Run the stcox command
        stcox `x' AGE SEX i.RACE if sample_final==1 & _t>1
