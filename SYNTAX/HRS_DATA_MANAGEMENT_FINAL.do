...
# Read the dataset
data <- read.csv("D:/16GBBACKUPUSB/BACKUP_USB_SEPTEMBER2014/SUMMER_STUDENT_2025/HRS_NHANES_HEI_EPICLOCKS_MORT/FINAL_DATA/HRS/cleaned_results_TABLE5_MODEL1.csv")

# Rename columns
colnames(data) <- c("Variable", "LnHR", "SE")

# Ensure numeric (important if CSV imported as character)
data$LnHR <- as.numeric(data$LnHR)
...
# Preserve order safely (no duplication issues)
data$Variable <- fct_rev(fct_inorder(data$Variable))
...
capture log using "E:\16GBBACKUPUSB\BACKUP_USB_SEPTEMBER2014\SUMMER_STUDENT_2025\NHANES_HRS_HANDLS_TLEPIGENMORT\OUTPUT\HRS_HEATMAPCOX_SENS.smcl",replace
...
foreach x of varlist zHorvathAgeEAA zHannumAgeEAA zPhenoAgeEAA zGrimAgeMortEAA zDunedinPoAm zhei2015_total_score zses { 
        ...
        * Run the stcox command
        stcox `x' AGE SEX i.RACE_ETHN if sample_final==1 & _t>1
